	...
	m_img(Vec2i(10, 10), ImageFormat::RGBA_Vec4f) // will get resized immediately
{
	...
	m_commonCtrl.addToggle(&m_useBruteForcePT, FW_KEY_NONE, "Use brute force path tracing", &clear_on_next_frame);
	m_commonCtrl.addToggle(&m_useMIS, FW_KEY_NONE, "Use MIS", &clear_on_next_frame);
...
bool App::handleEvent(const Window::Event& ev)
{
	...
	case Action_PlaceLightSourceAtCamera:
		...
		m_RTMode = false;
		constructTracer(); // Need to reconstruct the BVH since light location changed
...
void App::renderFrame(GLContext* gl)
{
	...
	if (worldToClip != previous_camera || clear_on_next_frame)
	{
		...
		m_pathtrace_renderer->stop();
		m_pathtrace_renderer->setSampler(m_useBruteForcePT ? Sampler_BRDF : (m_useMIS ? Sampler_MIS : Sampler_Light));
