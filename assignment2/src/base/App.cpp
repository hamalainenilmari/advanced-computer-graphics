	...
	m_bounceToVisualize (0)
{
	...
	m_commonCtrl.addToggle((S32*)&m_action, Action_ToggleSphericalHarmonics, FW_KEY_F1, "Extra: Render with spherical harmonics (F1)");
	m_commonCtrl.addToggle(&m_useQMC,										FW_KEY_F2,  "Extra: Toggle QMC (F2)");
...
void App::renderFrame(GLContext* gl)
{
	...
	gl->setUniform(prog->getUniformLoc("t"), m_timer.getElapsed());
	if (m_mesh) {
		gl->setUniform(prog->getUniformLoc("ripple"), (m_mesh->numVertices() == 31440) ? 1 : 0);
	}
	GLContext::checkErrors();
...
void main()
{
	...
	if (hasNormalTexture)
		normal = normalize(normalVarying); // EXTRA: read the normal map instead. Remember that you need to orient the read normal correctly as described in Assignment 1.
	else
	...
	// Wondering about a missing 1/pi? colorVarying does actually not carry irradiance, but irradiance/pi.
	// We divide the irradiance computed in Radiosity::vertexTaskFunc by pi in Radiosity::updateMeshColors before sending it to the GPU
	// Hence we get the radiance just by multiplying by diffuseColor (=rho) instead of rho/pi.
